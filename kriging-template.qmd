...
#st_crs(ca_dgw_sf)
ca_dgw_3857_sf <- st_transform(ca_dgw_sf, st_crs(ca_counties_sf)) %>%
  rename(depth = dgbs)

ggplot()+
  geom_sf(data = ca_counties_sf, color = 'grey', alpha=0.7)+
  geom_sf(data = ca_dgw_3857_sf, aes(color = depth))
...
sj_depth_sf <- ca_dgw_3857_sf %>% 
  st_intersection(sj_county_sf) # compare this to 
